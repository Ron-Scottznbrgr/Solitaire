...
public partial class Card : Node2D
{
	...
	private Boolean isSelected; //Is the card currently being dragged
	private Boolean isSelectable; //Is the card intractable
	...
	public override void _Ready()
	{
		...
		Area2D area = GetNode<Area2D>("CardImage/Area2D");

		//Subscribe to events
		area.InputEvent += SelectCard;
		area.AreaEntered += OnZoneEnter;
		area.AreaExited += OnZoneExit;
	...
	public override void _Process(double delta)
	{
		...
		if (isSelected)
		{
			if (!Global.playerHoldingMouse)
			{
				if (isDroppable)
				{
					targetZone.CardIntake(this);
					currentZone.RemoveCard(this);
					currentZone = targetZone;
				}
				else
				{
					this.GlobalPosition = previousPosition;
				}
				isSelected = false;
				isDroppable = false;
			}
	...
	public void ZoneTransfer(Node2D zone)
	{
		...
		previousPosition = new Vector2(zone.GlobalPosition.X, zone.GlobalPosition.Y);
	}
	...
	public void ZoneTransfer(Node2D zone, int yOffset)//for King Zones, stacks the cards.
	{
		...
		targetpos = new Vector2(zone.GlobalPosition.X, zone.GlobalPosition.Y + yOffset);
	...
	public void SetPreviousPosition(Vector2 pos)
	{
		previousPosition = pos;
	}

	public void SetCurrentZone(cardZone newZone)
	{
		GD.Print("New Card Zone: " + newZone.Name);
		currentZone = newZone;
	}
	...
	public void OnZoneEnter(Area2D area)
	{
		GD.Print("=========================================================");
		GD.Print("Card Entered Zone " + area.GetParent().Name);
		cardZone zone = (cardZone)area.GetParent();

		if (zone != null)
		{
			GD.Print("Card Zone: " + zone.Name);
			GD.Print("Current Card Zone: " + currentZone.Name);

			if (isSelected)
			{
				if (zone.RuleCheck((Card)this))
				{
					GD.Print("Success");
					targetZone = zone;
					isDroppable = true;
	...
	public void OnZoneExit(Area2D area)
	{
		GD.Print("Card Exited Zone " + area.GetParent().Name);
		targetZone = null;
		isDroppable = false;
	}
